...
class RoleAuthorityServer:
    def __init__(self, host=HOST, port=PORT):
        self.host = host
        self.port = port
        self.auth = Authority("RoleAuthority")
        self.pubkey_pem = pubkey_to_pem(self.auth.public_key)
        print(f"[RoleAuthority] Listening on {self.host}:{self.port} ...")
        print("[RoleAuthority] Available ops: get_pubkey, request_cert")
    ...
    def handle_client(self, conn: socket.socket, addr):
        try:
            ...
            if op == "get_pubkey":
                resp = {"status": "ok", "pubkey_pem": self.pubkey_pem, "issuer": "RoleAuthority"}
                f.write((json.dumps(resp) + "\n").encode()); f.flush()
                print(f"[RoleAuthority] Served get_pubkey to {addr}")
                ...
                f.write((json.dumps(resp) + "\n").encode()); f.flush()
                print(f"[RoleAuthority] Issued '{attribute}' to {user_id} for {addr}")
            ...
            print("[RoleAuthority] Error handling client:", e)
        finally:
            conn.close()
...
if __name__ == "__main__":
    RoleAuthorityServer().serve_forever()
